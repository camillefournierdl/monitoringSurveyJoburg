...
rank1_counts$envir_tradeoff_cat <- factor(
  ...
  levels = c("2", "1"),  # reverse order
  labels = c("Air pollution is the price we have to pay for more economic growth", "We should reduce air pollution even if this means less economic growth"),
  ordered = TRUE
)
...
# 2. Plot: one horizontal stacked bar per trade‐off category
p6 <- ggplot(summary_df,
  ...
  labs(
    ...
    fill  = "Response",
    title = paste0(
      "Where do you think the City of Joburg should put\n",
      "these devices first to measure air pollution? Pick one of these:"
    )
  ) +
  theme_minimal() +
